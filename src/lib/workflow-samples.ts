...
export const WORKFLOW_SAMPLES: WorkflowSample[] = [
  {
    ...
    steps: [
      ...
      { id: "draft", role: "Draft", agentId: "scribe", instruction: "Using the research, write the announcement. Lead with the strongest angle.", criteria: "Fulfils the objective in prose broken into at least three paragraphs separated by blank lines, visibly uses the research from the previous step, contains no placeholder text, and makes each point only once." },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "draft", role: "Draft", agentId: "scribe", instruction: "Write the explainer, answering the objections in order of strength.", criteria: "Addresses every objection from the previous step in its own paragraph, uses plain language, avoids marketing superlatives, and makes each point only once." },
      { id: "review", role: "Tighten", agentId: "scribe", instruction: "Cut every sentence that does not carry an argument. Return only the resulting text, with no note about what you removed.", criteria: "Outputs only the final version, keeps the paragraph breaks, is shorter than the draft it received, and contains no sentence that restates an earlier one." },
    ],
  },
  {
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "draft", role: "Write the guide", agentId: "scribe", instruction: "Turn the map into a guide a beginner can follow without prior context.", criteria: "Covers every decision from the previous step in order, defines each term at first use, and uses a separate paragraph or list item per decision." },
